...
class WatchDataManager: NSObject, WCSessionDelegate {
    ...
    private func createWatchContext(completionHandler: (context: WatchContext?) -> Void) {
        ...
        deviceDataManager.loopManager.getLoopStatus { (predictedGlucose, carbEffect, insulinEffect, recommendedTempBasal, lastTempBasal, lastLoopCompleted, insulinOnBoard, error) in
